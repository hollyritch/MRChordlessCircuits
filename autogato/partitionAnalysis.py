...
def checkEquivalenceClassSuperCore(c, eqClass, autocatalytic):
    """
    Classify elementary circuits into equivalence classes according their 
    vertices and edges.
...
def checkCoreInclusionAlternative(c, partialCores:list):
    # Return immediately if c is flagged as no core
    coreFlag = True
    for d in partialCores:
        if d<c:
            coreFlag=False
            break
    return coreFlag, c
...
def alternativeInclusionCheck(cores:set, realCores:set, coreList:list, threads:int):
    with concurrent.futures.ProcessPoolExecutor(max_workers=threads) as executor:
        for c in tqdm(cores, leave=False, total=len(cores)):
            for f in concurrent.futures.as_completed(executor.submit(checkCoreInclusionAlternative, c, coreList[i]) for i in range(len(coreList))):
                try:
                    coreFlag, c = f.result()
                    if coreFlag == False:
                        realCores.discard(c)
...
def checkUniquenessOfCores(cores:set):
    global noThreads
    realCores = set()
    global noCore

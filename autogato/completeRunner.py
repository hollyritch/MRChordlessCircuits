...
for k in tqdm(range(len(speciesList)), desc="Species"):
    ...
    # 2. Read Model
        ...
        for j in tqdm(range(len(pickleFileList)), leave = False, desc="Pickle-Files"):
            ...
            newOrder = "python partitionAnalysis.py" + " -x " + xmlFilePath + " -i " + pickleFilePath + " -b " + str(cutOffElementaryCircuits) + " -n True " + " -s " + str(species)+ " -t " + str(maxThreads) + " -e " + str(cutOffLargerCycles) +  coreString  + " -p" + " -o /scratch/richard/Autocatalysis/cycleData/"
            print(newOrder)

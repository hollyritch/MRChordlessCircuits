...
def assembleCython(equivClass:frozenset, equivClassValues: dict, elemE:dict, M:dict, circuitIdMrEdgeDict:dict, cutoff:int):
    """
...
def checkMatch(equivClassDict:dict, interEquivClassDict:dict):
    newMR = {}

    mr1 = equivClassDict["MR"]

    mr2 = interEquivClassDict["MR"]
    rm2 = interEquivClassDict["RM"]
...
def getIntersectingEquivClassesParallelCython(equivClass:frozenset, M:dict, circuitIdMrEdgeDict:dict):
    """
    Determine equivalence classes intersecting with the current class.
    """
    intersecEquivClasses = []
    changeE = {}
...
def assembleCythonCores(equivClass:frozenset,equivClassValues:dict, elemE:dict, M:dict, circuitIdMrEdgeDict:dict, cutoff:int, S:np.matrix, mID:dict, rID:dict):
    """
    Assemble new equivalence classes from a given class.
    """
            ...
            if plausible:
                ...
                if metzler == True:
                    ...
                    newEquivClasses[newFrozen] = {
                        ...
                        "Visited": False,
                        "Core": autocatalytic,
                        "Size": equivClassValues["Size"] + elemE[interEqCl]["Size"]
                    }
    return newEquivClasses, changeE
#############################
#############################
...
def computeSubstochasticMatrixForSetOfMREdgesCython(S:np.matrix, mID:dict, rID:dict , newEquivClass:frozenset):
    '''
        Determine CS matrix
...
def getIntersectingEquivClassesParallelCythonCores(equivClass:frozenset, equivClassValues:dict, M:dict,  circuitIdMrEdgeDict:dict, elemE:dict):

...
cutOffElementaryCircuits = 20
cutOffLargerCycles = 20
maxThreads = 24
coreString = " -c"
...
for k in tqdm(range(len(speciesList)), desc="Species"):
    ...
    # 2. Read Model
        ...
        for j in tqdm(range(len(pickleFileList)), leave = False, desc="Pickle-Files"):
            ...
            newOrder = "PYTHON_GIL=0 python partitionAnalysis.py" + " -x " + xmlFilePath + " -i " + pickleFilePath + " -b " + str(cutOffElementaryCircuits) + " -n True " + " -s " + str(species)+ " -t " + str(maxThreads) + " -e " + str(cutOffLargerCycles) +  coreString  + " -p" + " -o /scratch/richard/Autocatalysis/cycleData/"
            print(newOrder)
            ...
            if sys.platform == "darwin":
                ...
                os.system('/usr/bin/time -f "Command: %C\nTime   : %E\nRAM(kb): %M\nStatus : %x\n" -o ./Results/'+str(species)+'/outputTimeRAM_'+ str(cutOffLargerCycles)+ "_" + str(j) + '.txt ' +newOrder)
